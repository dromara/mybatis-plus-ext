...
import com.tangzc.mpe.annotation.actable.IgnoreUpdate;
import com.tangzc.mpe.annotation.actable.Index;
import com.tangzc.mpe.annotation.actable.Table;
import com.tangzc.mpe.annotation.actable.Unique;
...
public class SysMysqlCreateTableManagerImpl implements SysMysqlCreateTableManager {
    ...
    public void initPack() {
        if (StringUtils.isEmpty(pack)) {
            pack = SpringContextUtil.getBootPackage();
        }
    }
    ...
    public void createMysqlTable() {
        ...
        Set<Class<?>> classes = ClassScanner.scan(packs, Entity.class, Table.class, TableName.class, javax.persistence.Table.class);
            ...
            if (ColumnUtils.hasColumnAnnotation(field, clas)) {
                ...
                if (null != unique) {
                    ...
                    param.setFiledUniqueName("".equals(unique.value()) ?
                            uniPrefix + (uniqueValue.length == 0 ? ColumnUtils.getColumnName(field, clas) : stringArrFormat(uniqueValue))
                            : uniPrefix + unique.value());
                    param.setFiledUniqueValue(uniqueValue.length == 0 ?
                            Collections.singletonList(ColumnUtils.getColumnName(field, clas))
